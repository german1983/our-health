...
// Serving units
router.get('/serving-units', authenticate, async (req, res, next) => {
  try {
    const productId = req.query.productId as string;
    const units = await intakeService.getServingUnits(req.userId!, productId);
    res.json(units);
  } catch (err) {
    next(err);
  }
...
router.post('/serving-units', authenticate, validate(createServingUnitSchema), async (req, res, next) => {
  try {
    const unit = await intakeService.createServingUnit(req.userId!, req.body);
...
router.patch('/serving-units/:id', authenticate, validate(updateServingUnitSchema), async (req, res, next) => {
  try {
    const unit = await intakeService.updateServingUnit(req.params.id, req.userId!, req.body);
    res.json(unit);
  } catch (err) {
    next(err);
  }
...
router.delete('/serving-units/:id', authenticate, async (req, res, next) => {
  try {
    await intakeService.deleteServingUnit(req.params.id, req.userId!);
    res.status(204).end();
  } catch (err) {
    next(err);
  }
